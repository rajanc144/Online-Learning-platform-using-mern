...
const postCourseController = async (req, res) => {
  try {
    ...
    // Check if req.files exists and map it safely
    const S_content = req.files && req.files.length > 0 ? req.files.map((file) => file.filename) : [];

    // Create an array of sections
    const sections = [];
    if (S_title && S_description && S_content.length > 0) {
      ...
      return res.status(400).send({ success: false, message: "Section data is missing or incomplete" });
    }
